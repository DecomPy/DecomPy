...
# Printing to the console an instance of train size of 6.
x_train, y_train = TRAIN_SIZE(6)
sess.run(tf.global_variables_initializer())
print(sess.run(y, feed_dict={x: x_train}))

sess.run(tf.nn.softmax(tf.zeros([8])))
...
for i in range(10):
    ...
    weight = sess.run(W)[:,i]
    plt.title(i)
    plt.imshow(weight.reshape([30,30]), cmap=plt.get_cmap('seismic'))
    frame1 = plt.gca()
