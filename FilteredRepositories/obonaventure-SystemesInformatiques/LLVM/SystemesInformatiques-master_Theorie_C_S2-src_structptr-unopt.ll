; ModuleID = 'Repositories/obonaventure-SystemesInformatiques/Unfiltered/SystemesInformatiques-master_Theorie_C_S2-src_structptr.c'
source_filename = "Repositories/obonaventure-SystemesInformatiques/Unfiltered/SystemesInformatiques-master_Theorie_C_S2-src_structptr.c"
target datalayout = "e-m:o-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-apple-macosx10.14.0"

%struct.fraction = type { i32, i32 }

; Function Attrs: nounwind ssp uwtable
define i32 @main(i32, i8**) #0 {
  %3 = alloca i32, align 4
  %4 = alloca i32, align 4
  %5 = alloca i8**, align 8
  %6 = alloca %struct.fraction, align 4
  %7 = alloca %struct.fraction, align 4
  %8 = alloca %struct.fraction*, align 8
  %9 = alloca %struct.fraction*, align 8
  store i32 0, i32* %3, align 4
  store i32 %0, i32* %4, align 4, !tbaa !3
  store i8** %1, i8*** %5, align 8, !tbaa !7
  %10 = bitcast %struct.fraction* %6 to i8*
  call void @llvm.lifetime.start.p0i8(i64 8, i8* %10) #2
  %11 = bitcast %struct.fraction* %7 to i8*
  call void @llvm.lifetime.start.p0i8(i64 8, i8* %11) #2
  %12 = bitcast %struct.fraction** %8 to i8*
  call void @llvm.lifetime.start.p0i8(i64 8, i8* %12) #2
  %13 = bitcast %struct.fraction** %9 to i8*
  call void @llvm.lifetime.start.p0i8(i64 8, i8* %13) #2
  store %struct.fraction* %6, %struct.fraction** %8, align 8, !tbaa !7
  store %struct.fraction* %7, %struct.fraction** %9, align 8, !tbaa !7
  %14 = load %struct.fraction*, %struct.fraction** %8, align 8, !tbaa !7
  %15 = getelementptr inbounds %struct.fraction, %struct.fraction* %14, i32 0, i32 0
  store i32 1, i32* %15, align 4, !tbaa !9
  %16 = load %struct.fraction*, %struct.fraction** %8, align 8, !tbaa !7
  %17 = getelementptr inbounds %struct.fraction, %struct.fraction* %16, i32 0, i32 1
  store i32 2, i32* %17, align 4, !tbaa !11
  %18 = load %struct.fraction*, %struct.fraction** %9, align 8, !tbaa !7
  %19 = getelementptr inbounds %struct.fraction, %struct.fraction* %18, i32 0, i32 0
  store i32 1, i32* %19, align 4, !tbaa !9
  %20 = load %struct.fraction*, %struct.fraction** %9, align 8, !tbaa !7
  %21 = getelementptr inbounds %struct.fraction, %struct.fraction* %20, i32 0, i32 1
  store i32 4, i32* %21, align 4, !tbaa !11
  %22 = bitcast %struct.fraction** %9 to i8*
  call void @llvm.lifetime.end.p0i8(i64 8, i8* %22) #2
  %23 = bitcast %struct.fraction** %8 to i8*
  call void @llvm.lifetime.end.p0i8(i64 8, i8* %23) #2
  %24 = bitcast %struct.fraction* %7 to i8*
  call void @llvm.lifetime.end.p0i8(i64 8, i8* %24) #2
  %25 = bitcast %struct.fraction* %6 to i8*
  call void @llvm.lifetime.end.p0i8(i64 8, i8* %25) #2
  ret i32 0
}

; Function Attrs: argmemonly nounwind
declare void @llvm.lifetime.start.p0i8(i64, i8* nocapture) #1

; Function Attrs: argmemonly nounwind
declare void @llvm.lifetime.end.p0i8(i64, i8* nocapture) #1

attributes #0 = { nounwind ssp uwtable "correctly-rounded-divide-sqrt-fp-math"="false" "disable-tail-calls"="false" "less-precise-fpmad"="false" "no-frame-pointer-elim"="true" "no-frame-pointer-elim-non-leaf" "no-infs-fp-math"="false" "no-jump-tables"="false" "no-nans-fp-math"="false" "no-signed-zeros-fp-math"="false" "no-trapping-math"="false" "stack-protector-buffer-size"="8" "target-cpu"="penryn" "target-features"="+cx16,+fxsr,+mmx,+sahf,+sse,+sse2,+sse3,+sse4.1,+ssse3,+x87" "unsafe-fp-math"="false" "use-soft-float"="false" }
attributes #1 = { argmemonly nounwind }
attributes #2 = { nounwind }

!llvm.module.flags = !{!0, !1}
!llvm.ident = !{!2}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 7, !"PIC Level", i32 2}
!2 = !{!"clang version 7.0.0 (tags/RELEASE_700/final)"}
!3 = !{!4, !4, i64 0}
!4 = !{!"int", !5, i64 0}
!5 = !{!"omnipotent char", !6, i64 0}
!6 = !{!"Simple C/C++ TBAA"}
!7 = !{!8, !8, i64 0}
!8 = !{!"any pointer", !5, i64 0}
!9 = !{!10, !4, i64 0}
!10 = !{!"fraction", !4, i64 0, !4, i64 4}
!11 = !{!10, !4, i64 4}
